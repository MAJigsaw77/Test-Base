...
class FlxRuntimeShader extends FlxShader
{
	...
	var glVersion:String = #if (emscripten || mobile) "100" #else "120" #end;
	 ...
	 */
	public function new(fragmentSource:String = null, vertexSource:String = null, version:String = null):Void
	{
		if (version != null)
		{
			trace('Setting the version to ${version}');
			glVersion = version;
		}
		else
			trace('Setting the default version to ${glVersion}...');
	...
	function processFragmentSource(input:String):String
		return input.replace("#pragma header", BASE_FRAGMENT_HEADER).replace("#pragma body", BASE_FRAGMENT_BODY).replace("#pragma version", glVersion);
	...
	function processVertexSource(input:String):String
		return input.replace("#pragma header", BASE_VERTEX_HEADER).replace("#pragma body", BASE_VERTEX_BODY).replace("#pragma version", glVersion);
